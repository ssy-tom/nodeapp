...
let query = require('./mysql_login').query;

// var await = require('asyncawait/await');
let teacher_data = function (res, username, profession) {
    
    let classExist = function () {
                let getClassList = query(`SELECT * from class WHERE class_name="1508A"`);
                return getClassList
    };
    ...
    let getData = function (hasClass) {
        if(hasClass.length==0){
            res.send({status:201,des:"用户名不存在"});
                ...
                let getStudentList = query(`SELECT s.id,s.birthday,s.id_card,s.length_of_schooling,s.name,s.now_has_course,s.score,
...
LEFT JOIN market ma ON s.market_id=ma.id`);
                let getClassList = query(`SELECT id,class_name from class`);
                let rewards = query(`SELECT * FROM reward_and_punishment`);
                let courseList = query(`SELECT * FROM course`);
                let thisTeachingCycle = query(`SELECT cycle_name,start_time,end_time FROM teaching_cycle WHERE type=1`);
                let theClassData = query(`SELECT c.id,c.class_name,he.name,co.course_name,c.cScore,c.status,c.profession FROM class c 
...
where c.class_name="1508A" and c.profession=0`);
                let students = query(`SELECT stu.id,stu.name,stu.sex,stu.id_card,stu.birthday,stu.time_of_come_school,cl.class_name,ma.name market
